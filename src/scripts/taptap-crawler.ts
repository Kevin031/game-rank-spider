...
async function crawlTapTapRanking() {
  try {
    ...
    // 保存到文件 - 以日期命名
    const outputFile = path.join(dataDir, `taptap-hot-${today}.json`);
    fs.writeFileSync(
      outputFile, 
      JSON.stringify({
    ...
    for (const game of games) {
      try {
        ...
        console.log(`已保存: ${game.title} (ID: ${gameId}, 排名: ${game.position})`);
      } catch (error) {
    ...
    games.forEach((game, index) => {
      console.log(`${game.position}. ${game.title} (ID: ${game.taptap_id}) - 粉丝数: ${game.fans_count}`);
    });
