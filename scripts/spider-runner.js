    ...
    const db = new sqlite3.Database(dbPath, (err) => {
      ...
      db.run(`CREATE TABLE IF NOT EXISTS games (
        ...
        db.run(`CREATE TABLE IF NOT EXISTS tags (
          ...
          // 创建game_tags关联表
          db.run(`CREATE TABLE IF NOT EXISTS game_tags (
            game_id INTEGER,
            tag_id INTEGER,
...
async function saveGameToDb(db, game) {
  return new Promise((resolve, reject) => {
    // 先查询是否存在
    db.get('SELECT id FROM games WHERE taptap_id = ?', [game.taptap_id], async (err, row) => {
      if (err) {
        reject(err);
        return;
      }
        ...
        await new Promise((resolve, reject) => {
          db.run(
            `UPDATE games 
             ...
             WHERE taptap_id = ?`,
            [game.title, game.description, game.hits_total, 
             game.logo_url, game.banner_url, game.url, 
             game.fans_count, game.hits_total_val, game.wish_count,
             game.taptap_id],
        ...
        const result = await new Promise((resolve, reject) => {
          db.run(
            `INSERT INTO games (taptap_id, title, description, hits_total, 
                              ...
                              fans_count, hits_total_val, wish_count) 
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [game.taptap_id, game.title, game.description, game.hits_total, 
             game.logo_url, game.banner_url, game.url,
             game.fans_count, game.hits_total_val, game.wish_count],
            function(err) {
              if (err) reject(err);
      ...
      if (game.tags && game.tags.length > 0) {
        ...
        for (const tag of game.tags) {
          ...
          const tagRow = await new Promise((resolve, reject) => {
            db.get('SELECT id FROM tags WHERE taptap_id = ?', [tag.id],
